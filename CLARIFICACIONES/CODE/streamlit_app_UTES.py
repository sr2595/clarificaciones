...
if archivo:
    ...
    if df_utes_grupo.empty:
        ...
        df_utes_grupo_sorted = df_utes_grupo[[col_cif, col_nombre_cliente, col_sociedad]].drop_duplicates().sort_values(by=col_cif)
        opciones_utes = [
            f"{row[col_cif]} - {row[col_nombre_cliente]}" if row[col_nombre_cliente] else f"{row[col_cif]}"
        ...
        df_internas = df_internas[df_internas['IMPORTE_CORRECTO'] > 0]

        # --- Mantener máximo una factura por sociedad ---
        df_internas = df_internas.sort_values(col_fecha_emision).drop_duplicates(subset=[col_sociedad], keep='first')
